...
*/
    ...
    $('#add-more-gsk-packing-list').click(function () {
           ...
           $('#table-gsk-packing-list tbody tr:last td:eq(6) input').attr({'id' : 'tare_wt-'+rows, 'value' : tare_wt , 'onchange': 'grossWeightCalculation('+rows+')'});
           $('#table-gsk-packing-list tbody tr:last td:eq(7) input').attr({'id' : 'net_wt-'+rows, 'value' : net_wt , 'onchange': 'grossWeightCalculation('+rows+')'});
    ...
    function grossWtBag(index) {
        ...
        var grossWtBag = ( parseFloat(tare_wt) + parseFloat(net_wt) ) * 6;
    ...
    function grandCalculationgsk(obj) {
        ...
        if(obj == 1) {
            ...
            var intialise = 0;
            var loopCounter = rowCount;
        } else if(obj == 2) {
            var intialise = 1;
            var loopCounter = (rowCount- 1);
        } else {
            var intialise = 1;
            var loopCounter = rowCount;
        }
        ...
        for(i = intialise; i <= loopCounter; i++) {
                ...
                var grossTare = $('#table-gsk-packing-list tbody tr:last td:eq(6) input').val();
                totalTare = parseFloat(totalTare) + (parseFloat(grossTare) * 6);

                var grossNet = $('#table-gsk-packing-list tbody tr:last td:eq(7) input').val();
                totalNet = parseFloat(totalNet) + (parseFloat(grossNet) * 6);

                var grossWt = $('#table-gsk-packing-list tbody tr:last td:eq(8) input').val();
                totalGross = parseFloat(totalGross) + parseFloat(grossWt);

                var tareCoggurated = $('#table-gsk-packing-list tbody tr:last td:eq(9) input').val();
                totalTareCoggurated = parseFloat(totalTareCoggurated) + parseFloat(tareCoggurated);

                var grossWtCoggurated = $('#table-gsk-packing-list tbody tr:last td:eq(10) input').val();
                totalGrossWtCoggurated = parseFloat(totalGrossWtCoggurated) + parseFloat(grossWtCoggurated);

                var palletNo = $('#table-gsk-packing-list tbody tr:last td:eq(11) input').val();
                totalpalletNo = parseFloat(totalpalletNo) + parseFloat(palletNo);
          }
         
          $("#grand_tare").val(totalTare.toFixed(2));
          $("#grand_net").val(totalNet.toFixed(2));
          $("#grand_gross").val(totalGross.toFixed(2));
   ...
   function checkDigitOrNot(index) {
    ...
    $(function() {
         $("#from_date_1, #upto_date_1, #lc_date_1, #da_date_1 ,#po_date_1, .tentative_date, .actual_date, #expiry_date-1, #license_date-1, .eo_extend_date").datepicker({ 
           format:'dd-MM-yyyy',
        });
    });
