...
<ol class="breadcrumb">
   ...
   <li class="breadcrumb-item"><a href="<?= base_url().$this->router->fetch_class().'/da_plant'?>">DA Account Copy</a></li>
   </li>
</ol>
<div class="card mb-3">
    <div class="card-block">
        <div style="float: right;">
            ...
            <?php if(isset($findOne['cancelled']) && $findOne['cancelled'] =='0' ) {?>
                ...
                <button class="btn btn-danger" type="button" onclick="pdf('DA-Plant-copy')"><i class="fa fa-file-pdf-o"></i>&nbsp;PDF</button>

                <a class="btn btn-warning" href="<?= base_url().'logistics/download/'.$da_header['id']?>"><i class="fa fa-download"></i>&nbsp;Download Attachment</a>
            
            <?php } else { ?>
                    <p class="text-danger text-right">This DA has been cancelled.</p>
            <?php } ?>
			...
			<tr>
				<td><table width="100%" border="0" cellpadding="0" cellspacing="0" class="noBorder">
					...
					<tr>
						...
						<td width="34%">THIS COPY TO: <span>PLANT</span></td>
					</tr>
					<tr>
				...
				<td class="noPadding"><table class="noOuterBorder" width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						...
						<td width="54%" class="noPadding"><table width="100%" border="0" cellpadding="0" cellspacing="0" class="noBorder">
							<tr>
								...
								<td width="37%">Dated: <?= nice_date( $da_header['lc_date'],'Y-M-d' ) ?></td>
							</tr>
							<tr>
					...
					<tr>
						...
						<tr>
							...
							<tr>
								<td>Agent <span><?= $da_header['agent_name'] ?></span></td><td><p>Transport Mode to CHA <span><?= $da_header['transportmodetocha'] ?></span></p></td>
							</tr>
					...
					<tr>
						<td class="noPadding"><table class="invoice" width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<th width="20%" align="left">No. &amp; Kind of Packages</th>
								<th width="26%" align="center">Description of Goods</th>
								<th width="14%" style="text-align: right;">Quantity</th>
								<th width="20%" style="text-align: right;">Rate <br/>
								USD</th>
								<th width="20%" style="text-align: right;">Amount<br/>
								USD</th>
							</tr>
							...
							<tr>
								...
								<td align="left"><?= $daitems['product_name'].' HS CODE: '.$daitems['hscode'] ?></td>
					...
					<tr>
						<td>Amount Chargeable(in words)): <span>
						USD</span> <span  style="padding: 0 0 0 10px; width: 54% !important; display: inline-flex;"><?= convert_number($sum)?> Only </span><span style="float:right"><strong><?=  !empty($da_items) ? number_format((float) $sum , 2, '.', '') : '' ?></strong></span></td>
					</tr>
					<tr>
						<td class="noPadding"><table width="100%" border="0" cellpadding="0" cellspacing="0" class="noBorder">
							<tr>
					...
					<tr>
						<td class="noPadding"><table width="100%" border="0" cellpadding="0" cellspacing="0" class="invoice">
							<tr>
								...
								<th align="left">0-30 Days</th>
								<th align="left">31-60 Days</th>
								<th align="left">61-95 Days</th>
								<th align="left">96-120 Days</th>
								<th align="left">121-180 Days</th>
								<th align="left">181-365 Days</th>
								<th align="left">&gt;365 Days</th>
							</tr>
							 <?php if(!empty($da_out_standing)) { 
                                $counter = 0;
                                foreach($da_out_standing as $outstanding) { ?>
							<tr>
								<td class="text-left"><?= $outstanding['out_standing'] ?></td>
                                <td align="left"><?= $outstanding['ageing'] ?></td>
                                <td align="left"><?= $outstanding['0_30_days'] ?></td>
                                <td align="left"><?= $outstanding['31_60_days'] ?></td>
                                <td align="left"><?= $outstanding['61_95_days'] ?></td>
                                <td align="left"><?= $outstanding['96_120_days'] ?></td>
                                <td align="left"><?= $outstanding['121_180_days'] ?></td>
                                <td align="left"><?= $outstanding['181_365_days'] ?></td>
                                <td align="left"><?= $outstanding['GT365_days'] ?></td>
							</tr>
                            <?php }  }   else {?> 
                             <tr>
		                         <td align="left">&nbsp;</td>
					...
					<tr>
						<td class="noPadding"><table width="100%" border="0" cellpadding="0" cellspacing="0" class="noBorder">
							...
							<tr>
								<td>
								 <span class="text-primary" style="padding-left: 0;"><?= !empty($da_header['license_check_by']) ?  'CHECKED': '' ?></span>  Licensed Checked By <?= !empty($da_header['license_check_by']) ?  $da_header['license_check_by']: '' ?>
								</td>
								<td>
									<span class="text-primary" style="padding-left: 0; color:#000080 !important;"><?= !empty($da_header['prepared_by']) ?  'CHECKED': '' ?> </span><br/>Prepared By<br/><?= !empty($da_header['prepared_by']) ?  $da_header['prepared_by']: '' ?>
								</td>
								<td>
									<span class="text-primary" style="padding-left: 0; color:#000080 !important;"><?= !empty($da_header['checked_by']) ?  'CHECKED': '' ?></span> <br/>Checked By<br/><?= !empty($da_header['checked_by']) ?  $da_header['checked_by']: '' ?>
								</td>
								<td>
									<span class="text-primary" style="padding-left: 0; color:#000080 !important;"><?= !empty($da_header['ecgc_checked_by']) ?  'CHECKED': '' ?> </span>ECGC Checked By<?= !empty($da_header['ecgc_checked_by']) ?  $da_header['ecgc_checked_by']: '' ?>
								</td>
								<td>
								Ordered By<br/><?= !empty($da_header['ordered_by']) ?  $da_header['username']: '' ?>
								</td>
								<td>
									<span class="text-primary" style="padding-left: 0; color:#000080 !important;"><?= !empty($da_header['approved_by']) ?  'APPROVED': '' ?></span> <br/>Approved By<br/><?= !empty($da_header['approved_by']) ?  $da_header['approved_by']: '' ?>
							   </td>
							</tr>
						</table></td>
					</tr><tr>
						<td class="noPadding"><table width="100%" border="0" cellpadding="0" cellspacing="0" class="noBorder">
							<tr>
								<td colspan="4">A.D. Code Suggested by Finance &amp; Accounts Deptt.   <label style="border:1px solid #000; margin-left:50px; display:inline-block; width:250px; height:30px;" > <input type="text" style="width: 247px;border: none;"> </label></td>
							</tr>
							<td colspan="4">&nbsp;</td>
						</tr>
						<tr>
